...
import random
from core.regions import A1, A2, A3, A4, A5, A6, R
...
# helper function to generate destroyed patches (with holes)
def create_destroyed_patch(region):
    # get coords
    exterior_destroyed = list(region.exterior.coords)
    if region.interiors:
        interior_destroyed = [
            list(interior.coords) for interior in region.interiors
        ]  # noqa
    else:
        interior_destroyed = []
    ...
    combined_codes = (
        [Path.MOVETO]
        + [Path.LINETO] * (len(exterior_path) - 2)
        + [Path.CLOSEPOLY]  # noqa
    )
    ...
    for interior_path in interior_paths:
        combined_codes += (
            [Path.MOVETO]
            + [Path.LINETO] * (len(interior_path) - 2)
            + [Path.CLOSEPOLY]  # noqa
        )
...
def random_point(region, used_points, label_points, base_regions_coords):
    while True:
        ...
        # check if the point is in region and not too close to other things
        if random_point.within(region_buffered) and random_point.within(
            border_buffered
        ):
            try:
                used_point_distance = min(
                    random_point.distance(used_point)
                    for used_point in used_points  # noqa
                )
            except ValueError:
                used_point_distance = 0.3

            regions_lines = [
                LineString(region_coords)
                for region_coords in base_regions_coords  # noqa
            ]
            line_distance = min(
                random_point.distance(region_line)
                for region_line in regions_lines  # noqa
            )

            label_distance = min(
                random_point.distance(label_point)
                for label_point in label_points  # noqa
            )

            if (
                label_distance >= (basic_padding * 2)
                and used_point_distance >= basic_padding
                and line_distance >= basic_padding
            ):
                return random_point
    ...
    for arr in D["saved"]:
        for region in arr:
            if region not in regions and not region.interiors:
                ...
                index_point = random_point(
                    region, used_points, label_points, base_regions_coords
                )
                used_points.append(index_point)
            ax.text(
                index_point.x,
